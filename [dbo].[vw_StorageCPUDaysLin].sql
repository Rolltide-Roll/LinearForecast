...
/****** Object:  View [dbo].[vw_StorageCPUDaysLin]    Script Date: 1/31/2017 1:52:42 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[vw_StorageCPUDaysLin] AS
...
(
     ...
     (
      ...
      LEFT OUTER JOIN 
      (	SELECT * FROM [dbo].[StorageCPUShorttermForecast] t (NOLOCK) WHERE ClusterType = 'Storage') t
...
(
     ...
     LEFT OUTER JOIN 
     (	SELECT * FROM [dbo].[StorageCPUShorttermForecast] t (NOLOCK) WHERE ClusterType = 'Storage'
     ) t
     ON 
	 s.Region = t.Region
     GROUP BY 
